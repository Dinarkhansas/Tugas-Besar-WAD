...
@section('content')
    ...
    {{-- Edit Sebuah Buku --}}
    <form action="{{ route('pemesanan.update', $pemesanans->id) }}" method="post" >
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('id') is-invalid @enderror" id="id"
                        name="id" value="{{ old('id', $pemesanans->id) }}">
                    @error('id')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('jenis_layanan') is-invalid @enderror" id="jenis_layanan" name="jenis_layanan">{{ old('jenis_layanan', $pemesanans->jenis_layanan) }}</textarea>
                    @error('jenis_layanan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('id') is-invalid @enderror" id="id" name="id" required>
                        ...
                        @foreach ($penyediaLayanans as $row)
                            <option value="{{ $row->id }}" {{ $row->id == $pemesanans->id ? 'selected' : '' }}>
                                {{ $row->id }} - {{ $row->nama }}
                            </option>
                        @endforeach
                    </select>
